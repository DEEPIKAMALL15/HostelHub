...
const RoomDetails = () => {
    ...
    const checkAvailability = async() => {
        try{
            ...
            const {data}=await axios.post('/api/bookings/check-availability',{room:id,checkInDate,checkOutDate},{withCredentials:true})
            if(data.success){
                if(data.isAvailable){
                    setIsAvailable(true);
        ...
        try{
            ...
            if(!isAvailable){
                ...
                const {data} = await axios.post('/api/bookings/book',{room:id,checkInDate,checkOutDate,guests,paymentMethod: "Pay At Hotel"},{
                    headers: {Authorization: `Bearer ${await getToken()}`},withCredentials:true
                });
                if(data.success){
  ...
  return room && (
    <div className='py-28 md:py-32 px-4 md:px-16 lg:px-24 xl:px-32'>
        ...
        <form onSubmit={onSubmitHandler} className='flex flex-col md:flex-row items-start md:items-center justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-5 rounded-xl mx-auto mt-16 max-w-6xl'>
            <div className='flex flex-col flex-wrap md:flex-row items-start md:items-center gap-2 md:gap-2 text-gray-500'>
                 <div className='flex flex-col'>
            ...
            <button type='submit' className='bg-primary hover:bg-primary-dull active:scale-65 transition-all text-white rounded-md max-md:w-full max-md:mt-6 md:px-50 py-3 text-base cursor-pointer'>
        ...
        <div className='max-w-3xl border-y border-gray-300 my-14 py-10 text-gray-500'>
            <p>Guests will be allocated on the ground floor accoring to availabity. You get a comfortable Two Bedroom apartment has a true city feeling. The price quoted is for two guest, at the guest slot please mark the number of guests to get the exact price for groups. The guests will be allocated ground floarr according to availabity.You get a comfortable Two Bedroom apartment has a true city feeling. </p>
        </div>
