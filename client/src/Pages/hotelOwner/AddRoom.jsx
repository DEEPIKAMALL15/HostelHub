        ...
        {Object.keys(images).map((key)=>(
           <label htmlFor={`roomImage${key}`} key={key}>
            ...
            <input type='file' accept='image/*' id={`roomImage${key}`} hidden onChange={e=> {
                ...
                if(!file) return;
                const maxBytes = 1.5 * 1024 * 1024; // ~1.5MB per file
                if(file.size > maxBytes){
                  toast.error('Each image must be under ~1.5MB');
                  e.target.value = '';
                  return;
                }
      ...
      <div className='w-full flex max-sm:flex-col sm:gap-4 mt-4'>
          <div className='flex-1 max-w-48'>
                ...
                className= 'border opacity-70 border-gray-300 mt-1 rounded p-2 w-full' >
                  ...
                  <option value="Single Bed">Private Room with Attached Bathroom</option>
